# Multi-Agent Architecture — MicroStrategy to Power BI Migration
...
| **@extractor** | Parsing MicroStrategy source artifacts | `microstrategy_export/` extraction modules |
...
| **@generator** | Cross-cutting generation coordination | `output/`, `powerbi_import/`, `test_output/`, `test_output_nocal/` generators |
...
```
        ...
        │(MicroStrategy parse)│  │ (coord.)  │   │ (coord.)   │
        └──────┬─────┘   └─────┬─────┘   └─────┬──────┘
               │          ┌────┴────┐     ┌─────┴──────┐
               │          │         │     │            │
               │     ┌────▼───┐ ┌───▼───┐ ┌▼────────┐ ┌▼──────┐
...
| **Formula Correctness** | Valid syntax, correct conversion from MicroStrategy formulas |
